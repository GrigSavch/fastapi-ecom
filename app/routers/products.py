...
async def get_all_products(
    ...
    in_stock: bool | None = Query(
        None,
        description="true — товары в наличии, false — без остатка"
    ),
    ...
    filters: list[ColumnElement[bool] | BinaryExpression[bool]] = [
    ...
    if search:
        ...
        if search_value:
            ...
            rank_col = func.ts_rank_cd(
                ProductModel.tsv, ts_query).label("rank")
            # total с учётом полнотекстового фильтра
            total_stmt = select(func.count()).select_from(
                ProductModel).where(*filters)
    ...
    if rank_col is not None:
        ...
        rows = result.all()
        items: Sequence[ProductModel] | list = [row[0] for row in rows]    # сами объекты
